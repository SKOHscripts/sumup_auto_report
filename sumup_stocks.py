...
def compute_indicators(item: dict, state: dict, weekly_sales: dict, weeks_range: list) -> dict:
    """
    Calcule tous les KPIs pour un article donné.
    """
    sku = item["sku"]
    sales_by_week = weekly_sales.get(sku, {})

    # Ventes par semaine (0 si pas de vente)
    ...
    # Projection : moyenne glissante 4 semaines par défaut
    proj_next_week = round(avg_rolling4, 1)
    proj_4_weeks = round(avg_rolling4 * PROJECTION_WEEKS, 1)

    # Stock
    stock_on_hand = state.get("stock_on_hand", 0) or 0
    stock_reserved = state.get("stock_reserved", 0) or 0
    incoming_qty = state.get("incoming_qty", 0) or 0
    ...
    # Couverture
    if avg_rolling4 > 0:
        coverage_weeks = available_stock / avg_rolling4
    else:
        coverage_weeks = None
    ...
    # Quantité à commander (POC)
    target_stock = item.get("target_stock", 0) or 0
    reorder_point = item.get("reorder_point", 0) or 0
    safety_stock = item.get("safety_stock", 0) or 0
    qty_to_order = max(0, target_stock - (available_stock + incoming_qty)) if target_stock > 0 else 0

    # Statut
    if avg_rolling4 == 0 and available_stock == 0 and target_stock == 0:
        status = "N/A"
    elif coverage_weeks is None:
        status = "N/A"
    elif available_stock <= 0:
        status = "RISQUE RUPTURE"
    elif reorder_point > 0 and available_stock <= reorder_point:
        status = "A COMMANDER"
    elif safety_stock > 0 and available_stock <= safety_stock * 1.5:
        status = "SURVEILLANCE"
    else:
        status = "OK"
...
class StockPDF(FPDF):
    ...
    def section_title(self, title: str, color: tuple = None):
        ...
        self.cell(self._pw() - 5, 7, title.upper(), border=0, new_x="LMARGIN", new_y="NEXT")
        self.set_draw_color(*PALETTE["divider"])
        self.set_line_width(0.2)
        y = self.get_y()
...
def render_page_summary(pdf: StockPDF, all_kpis: list, week_label: str, weeks_range: list):
    ...
    for i, kpi in enumerate(sorted_kpis):
        ...
        pdf.cell(col_lbl,  row_h, pdf._safe(kpi["label"], 28),  border="B", align="L")
...
def render_article_page(pdf: StockPDF, kpi: dict):
    ...
    pdf.kpi_block([
        ...
        ("Stock entrant",    kpi["incoming_qty"]),
        ("ETA reappro",      kpi["incoming_eta"] or "N/A"),
        ("Stock securite",   kpi["safety_stock"]),
        ("Point de commande",kpi["reorder_point"]),
        ("Stock cible",      kpi["target_stock"]),
    ...
    pdf.section_title("Detail des ventes hebdomadaires")
...
def render_data_quality_page(pdf: StockPDF, unmapped: list, all_kpis: list):
    ...
    if unmapped:
        ...
        pdf.cell(col_qty,  6, "Qte totale",   border=0, align="R",
                 new_x="LMARGIN", new_y="NEXT")
        for i, u in enumerate(unmapped[:30]):
